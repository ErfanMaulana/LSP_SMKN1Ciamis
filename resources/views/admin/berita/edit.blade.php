...
<div class="form-card">
    <form action="{{ route('admin.berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <textarea name="konten" id="konten" class="form-input @error('konten') error @enderror">{{ old('konten', $berita->konten) }}</textarea>
            @error('konten')
                <div class="form-error">{{ $message }}</div>
            @enderror
        ...
        <div class="form-group">
            ...
            <div class="current-image" id="currentImageContainer">
                ...
                <p id="noImageText" style="display: none; color: #6b7280; font-size: 13px; font-style: italic;">Gambar tidak ditemukan. Upload gambar baru di bawah ini.</p>
            </div>
            @endif
            <input type="file" name="gambar" id="gambar" class="form-input @error('gambar') error @enderror" 
                   accept="image/jpeg,image/png,image/jpg,image/gif" style="margin-top: 12px;">
            @error('gambar')
                <div class="form-error">{{ $message }}</div>
            @enderror
