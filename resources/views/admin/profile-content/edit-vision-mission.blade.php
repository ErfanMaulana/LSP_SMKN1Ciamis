...
@section('page-title', 'Edit ' . ucfirst($type))

@section('content')
<div class="form-container">
    <div class="form-header">
        <div>
            <h2>Edit {{ ucfirst($type) }} LSP</h2>
            <p class="subtitle">Perbarui {{ $type === 'visi' ? 'visi' : 'misi' }} LSP</p>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.profile-content.vision-mission.update', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <textarea id="content" name="content" rows="6" 
                              ...
                              placeholder="Tulis konten {{ $type }} di sini...">{{ old('content', $item->content) }}</textarea>
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-circle"></i> Perbarui {{ ucfirst($type) }}
                    </button>
                </div>
            </form>
        </div>
