...
<style>
    ...
    .form-control { padding: 10px 14px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 14px; transition: all 0.3s; width: 100%; font-family: inherit; background: #f8fafc; }
    .form-control:focus { outline: none; border-color: #0073bd; background: white; box-shadow: 0 0 0 3px rgba(0, 115, 189, 0.1); }
    ...
    .form-grid-3 { display: grid; grid-template-columns: 1fr 2fr; gap: 16px; }
    .form-grid-4 { display: grid; grid-template-columns: 1fr 1fr 2fr; gap: 16px; }
    ...
    .section-label { font-size: 18px; font-weight: 600; color: #0F172A; margin: 0 0 8px; display: flex; align-items: center; gap: 10px; }
    .section-label:before { content: ''; width: 4px; height: 20px; background: #0073bd; border-radius: 2px; }
    .section-desc { font-size: 13px; color: #64748b; margin: 0 0 20px; }

    .group-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; margin-bottom: 18px; }
    .group-header { display: flex; align-items: flex-end; justify-content: space-between; gap: 16px; margin-bottom: 16px; }
    .group-title { display: flex; flex-direction: column; gap: 4px; color: #1e293b; }
    .group-title strong { font-size: 15px; }
    .group-title small { font-size: 12px; color: #64748b; }
    .group-units { display: flex; flex-direction: column; gap: 14px; }
    .group-header .form-group { margin-bottom: 0; min-width: 280px; flex: 1; }
    .group-name-slot.is-hidden { display: none; }
    .group-name-input { background: white; }
    .unit-group-label { min-height: 42px; border-style: dashed; }

    .unit-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 20px; margin-bottom: 16px; transition: all 0.3s ease; }
    ...
    .unit-number { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; background: #0073bd; color: white; border-radius: 8px; font-size: 13px; font-weight: 700; }
    .unit-title { font-size: 14px; font-weight: 600; color: #1e293b; display: flex; align-items: center; gap: 10px; }

    .elemen-card { background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; margin-bottom: 12px; margin-left: 16px; }
    .elemen-card.collapsed .elemen-content { display: none; }
    .elemen-card.collapsed .toggle-icon { transform: rotate(-90deg); }
    .elemen-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
    .elemen-number { display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px; background: #0073bd; color: white; border-radius: 6px; font-size: 11px; font-weight: 700; }
    .elemen-title { font-size: 13px; font-weight: 600; color: #374151; display: flex; align-items: center; gap: 8px; }

    .kriteria-item { display: flex; gap: 10px; align-items: flex-start; margin-bottom: 8px; margin-left: 16px; }
    .kriteria-number { min-width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; background: #fef3c7; color: #92400e; border-radius: 4px; font-size: 10px; font-weight: 700; flex-shrink: 0; margin-top: 7px; }
    .kriteria-item .form-control { flex: 1; }

    .remove-btn { width: 28px; height: 28px; border: none; background: transparent; color: #94a3b8; border-radius: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; flex-shrink: 0; font-size: 16px; }
    .remove-btn:hover { background: #fee2e2; color: #ef4444; }

    .add-btn { display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; border: 1px dashed #cbd5e1; background: transparent; border-radius: 6px; font-size: 12px; font-weight: 500; color: #64748b; cursor: pointer; transition: all 0.2s; margin-top: 8px; }
    ...
    @media (max-width: 768px) {
        ...
        .group-header .form-group { min-width: 0; width: 100%; }
...
<div class="form-container">
    <form id="skemaForm" method="POST" action="{{ route('admin.skema.store') }}">
        @csrf
        <div class="card">
            <div class="card-body">
                <div class="section-label">Data Skema</div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="section-desc">Tambahkan kelompok pekerjaan, unit kompetensi, elemen, dan kriteria unjuk kerja (KUK).</p>

                <div id="units-container">
                    <div class="group-card" data-group-index="0">
                        <div class="group-header">
                            <div class="group-title">
                                <strong>Kelompok Pekerjaan #1</strong>
                            ...
                            <button type="button" class="remove-btn remove-group-btn" title="Hapus kelompok" style="display:none;">
                            ...
                            <div class="unit-card" data-unit-index="0">
                                ...
                                <div class="form-grid-4">
                                    <!-- <div class="form-group">
                                        ...
                                        <input type="hidden" name="units[0][kelompok_pekerjaan]" class="unit-group-value">
                                        <div class="form-control" style="background:#f8fafc; color:#64748b; display:flex; align-items:center;">Akan mengikuti nama kelompok di atas</div>
                                    </div> -->
                                    <div class="form-group">
...
<script>
    ...
    function updateRemoveButtons() {
        ...
        groups.forEach(group => {
            ...
            const units = group.querySelectorAll('.unit-card');
            units.forEach(unit => {
        ...
        const standaloneUnits = document.querySelectorAll('#units-container > .unit-card');
        standaloneUnits.forEach(unit => {
            const removeBtn = unit.querySelector(':scope > .unit-header .remove-unit-btn');
            if (removeBtn) removeBtn.style.display = standaloneUnits.length > 1 ? 'flex' : 'none';
        });
    }
