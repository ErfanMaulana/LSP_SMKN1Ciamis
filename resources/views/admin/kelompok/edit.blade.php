...
@section('page-title', 'Edit Kelompok')
...
<form method="POST" action="{{ route('admin.kelompok.update', $kelompok->id) }}">
    @csrf @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <div class="dd-wrapper" id="skemaWrapper">
                        ...
                        <input type="hidden" name="skema_id" id="skemaIdInput"
                               value="{{ old('skema_id', $kelompok->skema_id) }}">
                    </div>
            ...
            <div id="asesorSection">
                <div class="dd-wrapper" id="asesorWrapper">
                    ...
                    <input type="hidden" name="asesor_id" id="asesorIdInput"
                           value="{{ old('asesor_id', $selectedAsesorId) }}">
                </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div id="asesiLocked" class="locked-box" style="display:none;">
            ...
            <div id="asesiSection">
                ...
                <div class="form-hint" style="margin-top:6px;">Hanya asesi yang memiliki skema sesuai yang ditampilkan. Asesi yang saat ini sudah ada di kelompok ini tetap ditampilkan.</div>
...
@php
    $initSkemaId   = old('skema_id', $kelompok->skema_id);
    $initAsesorId  = old('asesor_id', $selectedAsesorId);
    $initAsesiNiks = old('asesi_niks', $selectedAsesiNiks ?? []);
@endphp
...
function renderAsesiBadges() {
    ...
    // Also update checkbox names for redundancy
...
// ── Initialize with existing kelompok data ────────────────────────────────────
(function init() {
    const initSkemaId   = {{ $initSkemaId ?? 'null' }};
    const initAsesorId  = {{ $initAsesorId ?? 'null' }};
    const initNiks      = @json($initAsesiNiks);

    // Pre-fill selAsesis from init niks
    initNiks.forEach(nik => {
        const a = ASESIS.find(x => x.nik === nik);
        if (a) selAsesis.set(nik, a);
    });

    if (initSkemaId) {
        const el = document.querySelector(`#skemaList .dd-item[data-id="${initSkemaId}"]`);
        if (el) {
            curSkemaId = initSkemaId;
            document.getElementById('skemaIdInput').value = initSkemaId;
            document.getElementById('skemaTriggerText').textContent = el.dataset.nama;
            el.classList.add('selected');
            rebuildAsesorList(initSkemaId);
            rebuildAsesiList(initSkemaId);
        }
    }

    if (initAsesorId) {
        const a = ASESORS.find(x => x.id === initAsesorId);
        if (a) { selAsesorId = a.id; selAsesorNama = a.nama; selAsesorNoMet = a.no_met || '-';
            ...
            document.querySelectorAll('#asesorList .dd-item').forEach(el =>
                el.classList.toggle('selected', parseInt(el.dataset.id) === a.id));
            renderAsesorBadge();
        }
    }
})();
